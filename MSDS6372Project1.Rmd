...
ols_plot_cooksd_bar(log.model2.step)
ols_plot_resid_stand(log.model2.step)

#compare test ASE results between stepwise model and 
test = test[,-(1:2)]
testMSRP = test$MSRP # GET real result 
test = test[,-13] # Delete MSRP from test 
# Calculate stepwise test ASE first
pred.test.step = predict(log.model2.step,test)
...
pred.test.lasso = predict(log.model1,test)
pred.test.lasso = exp(pred.test.lasso)
test_ASE_LASSO = mean((testMSRP-pred.test.lasso)^2)
test_ASE_LASSO #499722642

# By comparison, it seems LASSO performs a bit worse than StepwiseAIC selected predictors...
```
